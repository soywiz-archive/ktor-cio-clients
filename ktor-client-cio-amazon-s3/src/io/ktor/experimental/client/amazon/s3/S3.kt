...
import io.ktor.experimental.client.amazon.auth.*
...
class S3File(val bucket: S3Bucket, val file: String) {
    ...
    suspend fun writeBytes(content: ByteArray, contentType: ContentType? = null, access: S3.ACL = S3.ACL.PRIVATE) = put(
        ByteArrayContentWithLength(content), contentType = contentType, access = access
    )
    ...
    suspend fun writeString(
        ...
        contentType: ContentType? = null,
        access: S3.ACL = S3.ACL.PRIVATE
    ) = writeBytes(content.toByteArray(charset), contentType = contentType, access = access)
}

fun S3Bucket.file(name: String) =
    S3File(this, name)

fun S3.bucket(name: String) =
    S3Bucket(this, name)
