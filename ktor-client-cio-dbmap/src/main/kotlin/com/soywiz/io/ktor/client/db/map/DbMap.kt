...
class TablesInfo(val db: DbClient) {
    ...
    fun <T : Any> get(clazz: KClass<T>): TableInfo<T> = infos.getOrPut(clazz) { TableInfo(db, clazz) } as TableInfo<T>
}

val DbClient.tablesInfo: TablesInfo by property { TablesInfo(this) }
